    ...
    while attempt < max_retries:
        try:
            ...
            for index, (news, time, date, link) in enumerate(zip(newsbox, times, newsbox_date, newslink)):
                ...
                news_data.append({"title": news.text.strip(), "date": date.text, "image": image_src, "link": link, "description" : paragraph_texts})
...
# news_data 리스트를 JSON 파일로 저장
with open(f'media/news/naver/{today_date}.json', 'w', encoding='utf-8') as file:
